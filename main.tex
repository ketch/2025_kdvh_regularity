...
\subsection{Hyperbolized forms of the KdV equation with viscousity}

\begin{align}
    u_t + u u_x + u_{xxx} = \mu u_{xx}
\end{align}
We study here different hyperbolic approximations of this equation. 
A first  hyperbolic approximation reads
\begin{subequations} \label{vbh}
\begin{align}
    u_t + u u_x  + w_x -\mu v_x & = 0 \\
    \tau v_t - v_x & = -w\\
        \tau w_t +  u_x & = v
%            \tau z_t - u_x & = -z 
\end{align}
\end{subequations}
This system is hyperbolic with eigenvalues $(\lambda_1, \lambda_2, \lambda_3 = (u/2 - \sqrt{u^2+4/\tau}/2, 1/\tau, u/2+ \sqrt{u^2+4/\tau}/2)$
The entropy for this system is $\eta(U) = u^2/2 + \tau (v^2+w^2)/$, so we have 
$$
...
However, we see that 
$$
\eta'(U)\cdot P(U) = [u\;, \tau v\;, \tau w] [0;, w\;, -v]^t =0 
$$
which  violates  the hypotheses of the Theorem.
