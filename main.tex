...
\begin{align}
   x & \to \epsilon x & t & \to \epsilon^{(k_p - 2k_f  + 1) (k_p - k_f)^{-1} } t \\
     &\tau  \to \epsilon^{(k_p - 2k_f  + 1) (k_p - k_f)^{-1} } \tau \\
...
\begin{remark}[Notable particular cases\label{rem:ss-particularcases}] The above result  admits same exceptional cases which should be commented on.
\begin{enumerate}
\item  When $k_f=(1+k_p)/2$  we can see that $n_t=1$, so time will not scale and the same (scaled) solutions will be obtained at the same time, albeit on different domains. 
\item When $k_f=k_p$ the scaling  \eqref{psys-scaling} becomes singular. In this case one could replace it by the modified scaling 
\begin{subequations}
\label{psys-scaling-t}
\begin{align}
   x & \to \epsilon^{(k_p - 2k_f  + 1)^{-1} (k_p - k_f) }  x & t & \to \epsilon  t \\
     &\tau  \to \epsilon  \tau \\
    u & \to \epsilon^{(k_p - 2k_f  + 1)^{-1}} u & v & \to \epsilon^{k_f(k_p - 2k_f  + 1)^{-1}} v.
\end{align}
\end{subequations}
reducing to
$$
...
x & \to    x & t & \to \epsilon  t \\
     &\tau  \to \epsilon  \tau \\
    u & \to \epsilon^{(1  - k_p)^{-1}} u & v & \to \epsilon^{k_p(1-k_p )^{-1}} v.
\end{aligned}
$$
So in this case it is space that will not scale, and the same (scaled) solutions will be obtained at the same positions, albeit at different times. 
\item  When $k_f=(1+k_p)/2$ and $k_f=k_p$, we have a unique solution which is the linear case $k_f=k_p=1$ which is not very interesting for our purposes.
\end{enumerate}
\end{remark}


\begin{remark}[Conditions of applicability of the scaling symmetry\label{rem:s-applicability}] To apply the scaling symmetry we would like to be able to pass from 
large values of $\tau$ with small data, to large data with small values of $\tau$. This requires that $n_x$ and $n_\tau$ should have different signs. This means that
 interesting  closures for the flux and pressure are those verifying 
\begin{equation}\label{eq:ss-applicatbility}
k_p - 2k_f  + 1 < 0.
\end{equation}
Note that this does not preclude the  case $k_f=k_p$, as long as $k_p>1$. In this case, scaling \eqref{psys-scaling-t} must be used.
\end{remark}
